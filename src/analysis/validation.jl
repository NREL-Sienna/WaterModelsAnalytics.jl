...
#   progress (tanks)
# * enable getting WNTR data frames separately and combined with WM dataframes
...
# solution object not currently being used to get num_time_step; could remove, but may want
# to change approach? JJS 4/7/22
function _get_times(wm_data::Dict{String,<:Any}, wm_solution::Dict{String,<:Any})
    ## this old code a priori presumed one-hour time steps
    # num_time_step = length(wm_solution["nw"]) # number of time steps
    # time_step = wm_data["time_step"] / 3600.0 # length per time step (hour)
    num_time_step = Int(wm_data["duration"]/wm_data["time_step"])
    time_step = wm_data["time_step"]
...
# # compare nodes -- legacy both WM and WNTR
...
## node dataframes, currently only WaterModels
function get_node_dataframe(node_id::String, wm_data::Dict{String,<:Any},
                            wm_solution::Dict{String,<:Any})
    num_time_step, time_step = _get_times(wm_data, wm_solution)
    pressure_watermodels = _get_node_wm(node_id, num_time_step, wm_solution)
    node_df = DataFrames.DataFrame(time = time_step:time_step:time_step*num_time_step,
                                   pressure_watermodels = pressure_watermodels)
    return node_df
end
function _get_node_wm(node_id::String, num_time_step::Int64,
                      wm_solution::Dict{String,<:Any})
    pressure_watermodels = Array{Float64,1}(undef,num_time_step)
    for t in 1:num_time_step
        pressure_watermodels[t] = wm_solution["nw"][string(t)]["node"][node_id]["p"]
    end
    return pressure_watermodels
end

## tank data frames, both WM and WNTR
function get_tank_dataframe(tank_id::String, wm_data::Dict{String,<:Any},
    ...
    tank_df = DataFrames.DataFrame(time = time_step:time_step:time_step*num_time_step,
...
# # compare pipes -- legacy WM and WNTR
# function get_pipe_dataframe(pipe_id::String, wm_data::Dict{String,<:Any},
#                             wm_solution::Dict{String,<:Any}, wntr_data::PyCall.PyObject,
#                             wntr_simulation::PyCall.PyObject)
#     num_time_step, time_step = _get_times(wm_data, wm_solution)
    
#     pipe_name = "pipe"*pipe_id # for wntr object (as created by WMA)
...
# pipe dataframe, WM only
function get_pipe_dataframe(pipe_id::String, wm_data::Dict{String,<:Any},
                            wm_solution::Dict{String,<:Any})
    num_time_step, time_step = _get_times(wm_data, wm_solution)
    flow_watermodels, hl_watermodels = _get_pipe_wm(pipe_id, num_time_step, wm_data,
                                                    wm_solution)
    pipe_df = DataFrames.DataFrame(time = time_step:time_step:time_step*num_time_step,
                                   flow_watermodels = flow_watermodels,
                                   hl_watermodels = hl_watermodels)
    return pipe_df
end
function _get_pipe_wm(pipe_id::String, num_time_step::Int64, wm_data::Dict{String,<:Any},
                      wm_solution::Dict{String,<:Any})
    flow_watermodels = Array{Float64,1}(undef,num_time_step)
    hl_watermodels = Array{Float64,1}(undef,num_time_step)
    node_fr = string(wm_data["nw"]["1"]["pipe"][pipe_id]["node_fr"])
    node_to = string(wm_data["nw"]["1"]["pipe"][pipe_id]["node_to"])
    for t in 1:num_time_step
        flow_watermodels[t] = wm_solution["nw"][string(t)]["pipe"][pipe_id]["q"]
        hl_watermodels[t] = wm_solution["nw"][string(t)]["node"][node_fr]["h"] -
            wm_solution["nw"][string(t)]["node"][node_to]["h"]
    end
    return flow_watermodels, hl_watermodels
end


# compare short pipes -- legacy WM and WNTR
...
# compare valves -- legacy WM and WNTR
...
# pump dataframe, WM only; power and cost calculations need work JJS 4/7/22
function get_pump_dataframe(pump_id::String, wm_data::Dict{String,<:Any},
                            wm_solution::Dict{String,<:Any})
    num_time_step, time_step = _get_times(wm_data, wm_solution)
    status_watermodels, flow_watermodels, head_gain_watermodels =
        _get_pump_wm(pump_id, num_time_step, wm_data, wm_solution)
    pump_df = DataFrames.DataFrame(time = time_step:time_step:time_step*num_time_step,
                                   status_watermodels = status_watermodels,
                                   flow_watermodels = flow_watermodels,
                                   head_gain_watermodels = head_gain_watermodels)
                                   #power_watermodels = power_watermodels,
                                   #cost_watermodels = cost_watermodels)
    return pump_df
end
function _get_pump_wm(pump_id::String, num_time_step::Int64, wm_data::Dict{String,<:Any},
                      wm_solution::Dict{String,<:Any})
    status_watermodels = Array{Float64,1}(undef,num_time_step)
    flow_watermodels = Array{Float64,1}(undef,num_time_step)
    head_gain_watermodels = Array{Float64,1}(undef,num_time_step)
    ...
    for t in 1:num_time_step
        status_watermodels[t] = wm_solution["nw"][string(t)]["pump"][pump_id]["status"]
        flow_watermodels[t] = wm_solution["nw"][string(t)]["pump"][pump_id]["q"]
        head_gain_watermodels[t] = round(status_watermodels[t])*
            (wm_solution["nw"][string(t)]["node"][node_to]["h"]-
            wm_solution["nw"][string(t)]["node"][node_fr]["h"])
        # power_watermodels[t] = status_watermodels[t]*
        #     compute_pump_power(pump_obj,flow_watermodels[t],head_gain_watermodels[t],
        #                        wntr_data)
        # energy_price = wm_data["nw"][string(t)]["pump"][pump_id]["energy_price"]*3600 # $/Wh
        # cost_watermodels[t] = power_watermodels[t]*time_step*energy_price
    end

    return status_watermodels, flow_watermodels, head_gain_watermodels
    #, power_watermodels, cost_watermodels
end
...
# pump_obj is a WNTR/python variable -- needs to be switched to WM (or redo this in entirety
# as suggested by comments abov)
